...
do
	...
	if file then
		...
		if file then
			...
			error("DefSteam: Failed to write steam_appid.txt! (it's needed to run a Steam game directly when not launched through Steam) in cd : " .. err)
		end
	end
end

if appid == 480 then
	print("DefSteam: Using dummy appid ('Spacewar' the steamworks example)")
	print("DefSteam: You have to modify ./steam_appid.txt to change it")
end
